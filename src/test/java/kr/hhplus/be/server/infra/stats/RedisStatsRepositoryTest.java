...
class RedisStatsRepositoryTest {
    ...
    @DisplayName("saveSalesProductsByOrder를 호출하면 파라미터로 들어온 orderProduct와 orderDate를 통해 key와 member를 생성하여 quantity만큼 score를 추가한다.")
    @Test
    void saveSalesProductsByOrder() {
        ...
        // when
        redisStatsRepository.saveSalesProductsByOrder(command.orderProducts(), command.orderDateTime());
